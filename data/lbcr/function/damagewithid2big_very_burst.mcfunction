...
execute at @e[type=#minecraft:mobs,distance=..5,tag=!spectator] if score @s team_number = @e[type=#minecraft:mobs,distance=..0.001,tag=!spectator,limit=1] team_number run tag @e[type=#minecraft:mobs,distance=..0.001,tag=!spectator,limit=1] add nodamage
