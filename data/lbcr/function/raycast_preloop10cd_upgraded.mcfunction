...
execute if predicate lbc:sneak if score #mode lbc.math matches 2 run tellraw @s [{"translate":"mode_instant"}]
execute if predicate lbc:sneak if score #mode lbc.math matches 3 run tellraw @s [{"translate":"mode_chain"}]
execute unless predicate lbc:sneak run function lbcr:raycast_preloop10cd_upgraded_pre