...
execute at @e[type=#minecraft:mobs,distance=..5,tag=!spectator] if score @e[type=#minecraft:mobs,distance=..0.01,tag=!spectator,limit=1] team_number = @s team_number run tag @e[type=#minecraft:mobs,distance=..0.01,tag=!spectator,limit=1] add nodamage
