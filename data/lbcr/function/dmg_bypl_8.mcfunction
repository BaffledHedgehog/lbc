...
execute if entity @s[tag=check] if score @s Lifetime matches 30.. if entity @e[distance=..10,dx=0,dy=0,dz=0,type=#mobs,tag=!nodamage,tag=!spectator,limit=1] at @a[gamemode=!spectator,nbt={SelectedItem:{components:{"prisma": true,"upgradable": true}}}] if score @a[distance=..0.001,limit=1,gamemode=!spectator] lbcID2 = @s lbcID2 as @a[distance=..0.001,limit=1,gamemode=!spectator] at @s run function lbc:other/magic_academy/can_upgrade
