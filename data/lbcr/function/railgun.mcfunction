...
execute if entity @s[tag=have_effects] if entity @e[type=#minecraft:mobs,type=!minecraft:creeper,distance=..3,tag=!owner,limit=1,tag=!strict_map_object] run function lbcr:railgun_effect with storage lbc.math
...
execute if score temp_distance lbc.math > @s Distance positioned ^ ^ ^1 unless entity @e[type=minecraft:marker,distance=..20,tag=stopper,limit=1] if score tmp lbc.math matches 300 facing entity @e[type=#minecraft:mobs,type=!minecraft:creeper,type=!minecraft:creeper,tag=!owner,tag=!spectator,sort=nearest,limit=1,tag=!strict_map_object] feet run function lbcr:railgun