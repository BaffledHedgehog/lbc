execute store result score tmp lbc.math run data get entity @s Inventory[{Slot:-106b}].tag.teleportid
particle portal ~ ~0.81 ~ 0 0.5 0 0.5 50 force
playsound entity.enderman.teleport master @a ~ ~ ~ 1 1
execute store result score @s lbcID run data get entity @s Inventory[{Slot:-106b}].tag.teleportid
tag @s add tp
...
execute at @a[gamemode=!spectator,tag=!tp] if score @a[tag=!tp,limit=1,gamemode=!spectator] lbcID = @s lbcID run advancement grant @s only lbc:true_advancements/swords/anal_whip
tag @s remove tp
