$data modify storage lbc.math name set from entity @e[type=marker,tag=stored_stats_vanity,limit=1] data.Attributes[$(tmp)].Name
...
execute if score tmp lbc.math matches ..8 run function lbc:workingitems/witch_gens/vanity_respawn_return_attributes_pre with storage lbc.math