...
execute anchored eyes positioned ^ ^ ^ if score tmp lbc.math matches 0 if entity @e[type=#arrows,distance=..6,limit=1,sort=nearest,nbt={item:{components:{"minecraft:potion_contents":{custom_effects:[]}}}}] run function lbc:workingitems/railgun_bow_effect_store
execute anchored eyes positioned ^ ^ ^ if score tmp lbc.math matches 0 if data entity @e[type=#arrows,distance=..6,limit=1,sort=nearest] item.components.minecraft:potion_contents.potion run function lbc:workingitems/railgun_bow_effect_store_glackmojumba
