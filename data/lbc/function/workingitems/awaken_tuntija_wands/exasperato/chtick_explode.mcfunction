...
execute if score tmp lbc.math matches 10.. at @a[gamemode=!spectator,nbt={SelectedItem:{components:{"minecraft:custom_data":{"upgradable": true,exasperato_wand:1b}}}}] if score @a[gamemode=!spectator,limit=1,distance=..0.001] lbcID2 = @s lbcID2 as @a[gamemode=!spectator,limit=1,distance=..0.001] at @s run function lbc:other/magic_academy/can_upgrade
summon tnt ~ ~ ~ {fuse:2s}
