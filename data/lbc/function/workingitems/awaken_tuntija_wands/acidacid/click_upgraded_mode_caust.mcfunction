...
scoreboard players operation @e[type=minecraft:marker,distance=..0.0001,tag=great_acid_caust,tag=!actived,limit=1] team_number = @s team_number
execute if score #tier lbc.math matches 1 run tag @e[type=minecraft:marker,distance=..0.0001,tag=great_acid_caust,tag=!actived,limit=1] add on_fire
