...
execute as @e[type=#minecraft:mobs,distance=..1.5,tag=!spectator] unless score @s lbcID2 = @a[tag=caster,tag=!spectator,limit=1] lbcID2 unless score @s team_number = @a[tag=caster,tag=!spectator,limit=1] team_number store result score *hit_red lbc.math run damage @s 6 magic by @a[tag=caster,tag=!spectator,limit=1]
