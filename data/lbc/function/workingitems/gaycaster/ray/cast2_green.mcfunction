...
execute as @e[type=#minecraft:mobs,dx=0,dy=0,dz=0,distance=..5,tag=!spectator,limit=1] unless score @s lbcID2 = @a[tag=caster,tag=!spectator,limit=1] lbcID2 unless score @s team_number = @a[tag=caster,tag=!spectator,limit=1] team_number at @s run function lbc:workingitems/gaycaster/ray/cast2_green_entity
