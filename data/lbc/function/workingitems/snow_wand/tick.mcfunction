...
execute at @s if score @a[limit=1,tag=nodamage,nbt={SelectedItem:{components:{"minecraft:custom_data":{"snow_wand": true,"upgradable": true}}}},nbt=!{SelectedItem:{components:{"minecraft:custom_data":{"upgrade_available": true}}}}] lbcID2 = @s lbcID2 as @a[limit=1,tag=nodamage,tag=!spectator] store result score tmp lbc.math if entity @e[distance=..5,type=#mobs,tag=!nodamage,scores={frozen=1..},tag=!spectator]
