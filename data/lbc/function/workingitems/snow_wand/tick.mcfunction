...
execute at @s if score @a[limit=1,tag=nodamage,nbt={SelectedItem:{components:{snow_wand:1b,upgradable:1b}}},nbt=!{SelectedItem:{components:{upgrade_available:1b}}}] lbcID2 = @s lbcID2 as @a[limit=1,tag=nodamage] store result score tmp lbc.math if entity @e[distance=..5,type=#mobs,tag=!nodamage,scores={frozen=1..}]
