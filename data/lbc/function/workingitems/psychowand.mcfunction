...
execute if entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{"upgradable": true}}}}] if score #random6 lbc.math matches 1..3 if entity @e[type=#mobs,distance=0.01..30,tag=!spectator,limit=1,nbt={active_effects:[{id:"minecraft:blindness"}]},scores={lookback=1..,scared=1..}] run function lbc:other/magic_academy/can_upgrade
