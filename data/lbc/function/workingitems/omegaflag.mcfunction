...
tellraw @a {"score": {"name": "tmp1","objective": "lbc.math"}}
...
execute as @e[type=#minecraft:mobs,tag=!spectator,predicate=!lbc:nexus] run damage @s 6 cactus by @p[gamemode=!spectator,tag=caster]
execute store result score tmp2 lbc.math run data get entity @s Health 1
tellraw @a {"score": {"name": "tmp2","objective": "lbc.math"}}
