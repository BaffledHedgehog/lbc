...
execute at @e[type=#mobs,tag=!spectator,predicate=!lbc:nexus] run particle minecraft:entity_effect ~ ~1 ~ 0.5 1 0.5 0.5 100 normal
...
execute if entity @s[nbt={SelectedItem:{components:{"minecraft:custom_data":{upgradable:1b}}}}] if score tmp lbc.math = tmp1 lbc.math run function lbc:other/magic_academy/can_upgrade
tag @s remove caster
