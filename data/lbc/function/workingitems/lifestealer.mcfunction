...
execute store result score playersaround lbc.math if entity @e[type=#minecraft:mobs,distance=0.1..20,tag=!spectator,tag=!nomagic_active]
playsound minecraft:entity.cat.hiss master @a ~ ~ ~ 1.5 2
execute unless entity @s[tag=have_stand,tag=stand_clocks_active] as @e[type=#minecraft:mobs,distance=0.01..20,tag=!nomagic_active,tag=!spectator] run damage @s 1 cactus
execute if entity @s[tag=have_stand,tag=stand_clocks_active,scores={overheaven=..0}] as @e[type=#minecraft:mobs,distance=0.01..20,tag=!nomagic_active,tag=!spectator] run damage @s 5 cactus
execute if entity @s[tag=have_stand,tag=stand_clocks_active,scores={overheaven=1..}] as @e[type=#minecraft:mobs,distance=0.01..20,tag=!nomagic_active,tag=!spectator] run damage @s 10 cactus
tellraw @a[gamemode=!spectator,distance=0.1..20,tag=!nomagic_active] {"translate":"player_vampirism"}
