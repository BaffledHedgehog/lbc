...
execute as @e[type=#minecraft:raycasters,distance=..2,tag=shield_blockable] positioned ^ ^ ^-1 if score @s team_number = @p[gamemode=!spectator] team_number run tag @s add donotblock
execute as @e[type=#minecraft:raycasters,distance=..2,tag=shield_blockable2] positioned ^ ^ ^-1 if score @s team_number = @p[gamemode=!spectator] team_number run tag @s add donotblock
