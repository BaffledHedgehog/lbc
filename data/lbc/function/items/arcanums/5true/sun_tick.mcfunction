...
execute if items entity @s weapon.mainhand *[minecraft:custom_data~{sun_arcanum:1}] if items entity @s weapon.offhand *[minecraft:custom_data~{sun_arcanum:1}] as @e[distance=0.1..15,type=#mobs,tag=!spectator,predicate=!lbc:same_team] run damage @s 10 on_fire by @p[gamemode=!spectator,tag=sun_active,distance=..1]
