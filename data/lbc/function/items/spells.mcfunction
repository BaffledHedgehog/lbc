...
execute at @s[tag=!nomagic_active,nbt={Inventory:[{components:{"minecraft:custom_data":{raycastspell:1}},Slot:-106b}]}] unless entity @a[gamemode=!spectator,distance=..20,tag=nomagic_active] unless score lefthand swrg.math matches 1 run function lbc:workingitems/spell_left

execute at @s[tag=!nomagic_active,nbt={Inventory:[{components:{"minecraft:custom_data":{magicwand:1}},Slot:-106b}]}] unless entity @a[gamemode=!spectator,distance=..20,tag=nomagic_active] unless score lefthand swrg.math matches 1 run function lbc:workingitems/spell1_left

execute at @s[tag=!nomagic_active,nbt={Inventory:[{components:{"minecraft:custom_data":{summonerwand:1}},Slot:-106b}]}] unless entity @a[gamemode=!spectator,distance=..20,tag=nomagic_active] unless score lefthand swrg.math matches 1 unless entity @e[type=minecraft:marker,distance=..20,tag=stopper_magic,limit=1] run function lbc:workingitems/spellsummoner_left
