...
execute at @s[tag=!nomagic_active,nbt={SelectedItem:{components:{"summonerwand": true}}}] unless entity @a[gamemode=!spectator,tag=nomagic_active,distance=..20] unless entity @e[type=marker,tag=stopper_magic,distance=..20,limit=1] run function lbc:workingitems/spellsummoner
...
execute at @s[tag=!nomagic_active,nbt={Inventory:[{Slot:-106b,components:{"summonerwand": true}}]}] unless entity @a[gamemode=!spectator,tag=nomagic_active,distance=..20] unless score lefthand swrg.math matches 1 unless entity @e[type=marker,tag=stopper_magic,distance=..20,limit=1] run function lbc:workingitems/spellsummoner_left
