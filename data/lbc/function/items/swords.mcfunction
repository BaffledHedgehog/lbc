...
execute at @s[nbt={SelectedItem:{components:{mortis:1b}}}] at @e[nbt={HurtTime:10s},limit=1,distance=..7,tag=!spectator] rotated as @s rotated ~180 0 run function lbc:workingitems/mortis
execute at @s[nbt={SelectedItem:{components:{mortis:1b}}}] facing entity @e[nbt={HurtTime:10s},limit=1,distance=..7,tag=!spectator] eyes rotated ~ 0 run function lbc:mortisbreak
...
execute at @s[nbt={SelectedItem:{components:{holocaust:1b}}}] run effect give @e[type=#mobs,nbt={HurtTime:10s},distance=0.01..7,gamemode=!spectator] dolphins_grace 20 111
