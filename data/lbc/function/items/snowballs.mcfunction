...
execute anchored eyes positioned ^ ^ ^ as @e[type=snowball,limit=1,sort=nearest] if data entity @s Item.components.minecraft:custom_data{"special": true} run function lbc:projectiles/specials
execute if data entity @e[type=snowball,limit=1,sort=nearest] Item.components.minecraft:custom_data{"gadget": true} run function lbc:projectiles/eggs
