...
execute as @e[type=item,distance=0..,nbt={Item:{components:{"nexus_wire_red": true}}}] at @s if entity @e[type=item,distance=..3,nbt={Item:{components:{"nexus_wire_blue": true}}}] if entity @e[type=item,distance=..3,nbt={Item:{components:{"nexus_wire_black": true}}}] run function lbc:other/nexus/craft_white_wire
