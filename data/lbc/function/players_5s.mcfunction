execute if entity @s[nbt={Inventory:[{components: {"minecraft:custom_data":{"heall": true}}}]}] unless entity @e[type=marker,tag=stopper_magic,limit=1,distance=..20] run function lbc:workingitems/regeneration_belt
execute at @s[nbt={Inventory:[{Slot:-106b,components: {"minecraft:custom_data":{"deviltotem":true}}}]}] run function lbc:other/deviltotem0
execute at @s[nbt={SelectedItem:{components: {"minecraft:custom_data":{"deviltotem":true}}}}] run function lbc:other/deviltotem0
