...
execute store result entity @e[type=minecraft:item,distance=..1,sort=nearest,limit=1,nbt={Item:{components:{"minecraft:custom_data":{tpid:0}}}}] Item.components."minecraft:custom_data".tpid int 1 run scoreboard players get *id.cloud lbcID