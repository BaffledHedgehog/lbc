...
execute as @a[gamemode=!spectator,tag=!have_stand] at @s unless items entity @s container.* *[minecraft:custom_data~{stand_clocks:1}] unless items entity @s weapon.* *[minecraft:custom_data~{stand_clocks:1}] unless entity @e[type=minecraft:marker,distance=..0.1,tag=teleport_nearest_here] run summon minecraft:marker ~ ~ ~ {Tags:["teleport_nearest_here"]}
