...
execute as @e[type=#minecraft:raycasters,tag=area_3s] at @s facing entity @e[type=#minecraft:mobs,tag=!spectator,tag=!rcdenyalways,tag=!friendly_tuntija,sort=nearest,limit=1,tag=!strict_map_object] eyes unless dimension minecraft:nexus unless dimension minecraft:imprinted run function lbc:raycast/select3s
