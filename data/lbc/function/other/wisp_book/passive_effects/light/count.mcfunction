...
execute as @e[type=minecraft:item_display,tag=wisp,tag=wisp_light] if score @s lbcID2 = @a[tag=check_id,limit=1] lbcID2 run scoreboard players add *temp1 lbc.math 1
