...
execute as @e[dx=0,dy=0,dz=0,distance=..2,type=#mobs,tag=!spectator,tag=!damager] as @a[tag=damager,limit=1,distance=..20] run function lbc:other/wisp_book/passive_effects/air/zap/hit_entity
execute if score *test_ray Distance matches 50 run function lbc:other/wisp_book/passive_effects/air/zap/end
#particle dust{color:[0.0,0.7,1.0],scale:1} ~ ~ ~ 0 0 0 0 1 force
