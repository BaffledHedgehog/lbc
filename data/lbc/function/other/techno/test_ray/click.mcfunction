...
summon minecraft:item_display 0 -800 0 {item:{id:"minecraft:repeating_command_block",components:{"minecraft:item_model":"lbc:laser_minigun_ray"},count:1},Tags:["test_ray"]}
scoreboard players operation @e[type=minecraft:item_display,tag=test_ray,tag=!done,limit=1] lbcID2 = @s lbcID2
scoreboard players operation @e[type=minecraft:item_display,tag=test_ray,tag=!done,limit=1] team_number = @s team_number
