...
execute as @e[type=#minecraft:mobs,dx=0,dy=0,dz=0,distance=..2,tag=!spectator] unless score @s lbcID2 = *check_id lbcID2 unless score @s team_number = *check_team lbcID2 as @a[gamemode=!spectator] if score @s lbcID2 = *check_id lbcID2 run function lbc:other/techno/laser_minigun/ray/hit_entity
