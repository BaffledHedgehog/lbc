...
execute if score @s lbcID2 = @e[tag=raycaster,limit=1] lbcID2 as @e[tag=raycaster,limit=1] at @s anchored eyes positioned ^ ^ ^ as @e[type=item_display,limit=1,tag=laser_ray,tag=!done] run function lbc:other/techno/laser_minigun/ray/tp
