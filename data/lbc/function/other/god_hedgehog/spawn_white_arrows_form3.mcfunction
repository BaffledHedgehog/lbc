

execute store result storage lbc.math i int 180 run scoreboard players get ticker lbc.math
execute store result storage lbc.math j int 1 run scoreboard players get 0 lbc.math
execute store result storage lbc.math k int 1 run random value -10..10
execute store result storage lbc.math r int 1 run scoreboard players get 0 lbc.math
function lbc:other/god_hedgehog/spawn_white_arrows1 with storage lbc.math
