...
execute at @a[gamemode=!spectator] if score @a[distance=..0.01,gamemode=!spectator,limit=1] lbcID2 = @s lbcID2 run tag @a[distance=..0.01,gamemode=!spectator,limit=1] add owner
...
tag @a remove owner
playsound item.trident.riptide_3 master @a ~ ~ ~ 2.5 0.5