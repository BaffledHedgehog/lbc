...
execute if score *chain lbc.math matches 1.. anchored eyes positioned ^ ^ ^ facing entity @e[type=#minecraft:mobs,distance=0.01..20,tag=!chained,tag=!damager,tag=!nodamage,tag=!spectator,sort=nearest,limit=1] eyes as @e[tag=damager,limit=1,type=#mobs,distance=..20] run function lbc:other/shock_effect/zap/cast
