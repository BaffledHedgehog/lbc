...
execute store result score value lbc.math run data get entity @s Item.components."minecraft:custom_data".value 1
execute as @e[type=minecraft:item,distance=..1,sort=nearest,limit=1,nbt={Item:{components:{"minecraft:custom_data":{libspell:1}}}}] run function lbc:other/craft_uniwand_with_grimoire_tier_check
execute store result entity @s Item.components."minecraft:custom_data".value int 1 run scoreboard players get value lbc.math
function lbc:other/craft_uniwand_with_grimoire_value_check
