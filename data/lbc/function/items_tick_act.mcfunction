execute if entity @s[tag=drop] run return run function lbc:drops/specialdrops
...
execute if entity @s[tag=malevich] if predicate lbc:chance10 as @e[type=minecraft:item,distance=..3,nbt=!{Item:{components:{"minecraft:custom_data":{malevich:1}}}}] at @s run return run function lbc:items_tick_act_m
execute if entity @s[tag=glowing] run particle minecraft:end_rod ~ ~ ~ 0 0 0 0.2 2 normal
execute if entity @s[tag=penis_bomb] if entity @e[type=#minecraft:mobs,type=!minecraft:player,distance=..1.2,tag=!no_penis] run return run function lbc:workingitems/penis_mob
